...
NUM_CLASSES = 10#类别个数
NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = 50000#训练集样本数
NUM_EXAMPLES_PER_EPOCH_FOR_EVAL = 10000#测试集样本数
...
def read_cifar10(filename_queue):
  ...
  result.width = 32
  result.depth = 3
  image_bytes = result.height * result.width * result.depth#每张图片所占字节数
  # Every record consists of a label followed by the image, with a
  # fixed number of bytes for each.
  record_bytes = label_bytes + image_bytes#每次读取的字节数
