...
def graphCreate() :
    with tf.Session() as sess :
        ...
        var2 = tf.Variable( 20 , name='var2_to_update' )#实参name='var2'指定了操作名，该操作返回的张量名是在
                           #'var2'后面:0 ,即var2:0 是返回的张量名，也就是说变量
                           # var2的名称是'var2:0'

        var3 = tf.Variable( 30 , name='var3_to_update' )
        var4 = tf.Variable( 40 , name='var4_to_update' )
        ...
        with open(os.path.join(model_dir, 'operation2.csv'),'wt' ) as f:
            s = 'name,type,output\n'
            f.write( s )
            for o in oper:
                s = o.name
...
def graphCreateFromPb():
    with tf.Graph().as_default():
        with tf.Session() as sess:
            var1 = tf.placeholder ( tf.int32 , name='var1' )
            var2 = tf.Variable( 2 , name='var2_to_update' )#实参name='var2'指定了操作名，该操作返回的张量名是在
                               #'var2'后面:0 ,即var2:0 是返回的张量名，也就是说变量
                               # var2的名称是'var2:0'

            var3 = tf.Variable( 3 , name='var3_to_update' )
            var4 = tf.Variable( 4 , name='var4_to_update' )
...
#                print(op.name,"-----",op.values())
                for value in op.values():
                    if "to_update" not in value.name or "pb1/" not in value.name or "read" in value.name:
                        continue
                    print(value.name)
                    update_op=tf.assign(sess.graph.get_tensor_by_name(value.name.split("pb1/")[1]),sess.graph.get_tensor_by_name(value.name))
                    pb_copy_ops.append(update_op)
            ...
            print('sum_operation result is Tensor ' , sess.run( sumOps , feed_dict={var1:1}) )
            constant_graph = graph_util.convert_variables_to_constants( sess, sess.graph_def , ['sum_operation'] )
            with open(os.path.join(model_dir,"graphB2.pb"), mode='wb') as f:
                f.write(constant_graph.SerializeToString())
...
def graphGet() :
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess :
            ...
            v2 = sess.graph.get_tensor_by_name('var2_to_update:0' )
            v3 = sess.graph.get_tensor_by_name('var3_to_update:0' )
            v4 = sess.graph.get_tensor_by_name('var4_to_update:0' )
